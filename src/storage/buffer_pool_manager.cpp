...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if(exist_page(page_id, &target_fid)){
        replacer_->pin(target_fid);
        pages_[target_fid].pin_count_++;
        return &pages_[target_fid]; 
    } else if(find_victim_page(&target_fid)){
        Page* victim_page = &pages_[target_fid];
        update_page(victim_page, page_id, target_fid);
        disk_manager_->read_page(page_id.fd, 
                                page_id.page_no, 
                                victim_page->data_, 
                                PAGE_SIZE);
        replacer_->pin(target_fid);
...
/**
 * @description: （自写辅助函数）在page_table_中查找page_id对应的帧fid
 * @return {bool} 若page不存在则返回false，查找到则返回true
 * @param {PageId} page_id 要查找的page的Id
 * @param {frame_id_t} fid 查找到的page的所在帧
 */
bool BufferPoolManager::exist_page(PageId page_id, frame_id_t* fid){
    auto it = page_table_.begin();
    if((it = page_table_.find(page_id)) == page_table_.end()){
        return false;
    }
    *fid = it->second;
    return true;
